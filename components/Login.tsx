  ...
  return (
    <div className="flex flex-col h-full w-full bg-background-light dark:bg-background-dark overflow-y-auto">
      <div className="w-full h-64 relative overflow-hidden rounded-b-[2rem] shrink-0">
        {/* Gradient overlay to maintain the "cloudy/faded" design at the bottom for text readability */}
        <div className="absolute inset-0 bg-gradient-to-t from-background-dark via-transparent to-transparent z-10 opacity-90"></div>
        
        {/* Updated background image to use the local 'imagen.jpg' file as requested */}
      ...
      <div className="flex flex-col items-center pt-6 px-8 shrink-0">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-xs mt-2 text-center">Acceso exclusivo para personal autorizado</p>
      </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className={`w-full bg-white dark:bg-surface-dark border ${error ? 'border-red-500' : 'border-none'} rounded-xl py-3.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-primary shadow-sm transition-all`}
              />
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input type="password" placeholder="••••••••" className="w-full bg-white dark:bg-surface-dark border-none rounded-xl py-3.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-primary shadow-sm" />
            </div>
          </div>
          ...
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center text-xs uppercase"><span className="bg-background-light dark:bg-background-dark px-2 text-slate-500 font-bold">Acceso Rápido (Demo)</span></div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            {USERS.map(user => (
                <button 
                    ...
                    className="flex items-center gap-3 p-3 bg-white dark:bg-surface-dark rounded-xl border border-slate-100 dark:border-slate-800 hover:border-primary/50 text-left transition-all"
                >
                    <div className="h-8 w-8 rounded-full bg-cover bg-center shrink-0" style={{ backgroundImage: `url('${user.avatar}')` }}></div>
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold truncate">{user.name}</p>
                        <p className="text-[10px] text-slate-500 truncate">{user.role}</p>
                    </div>
                    <span className="material-symbols-outlined text-slate-300 text-sm">login</span>
                </button>
            ))}
          </div>
        </div>
